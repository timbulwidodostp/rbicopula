...
/****************************************************************/
/*     rbicopula tmeffects: rbicopula postestimation command	*/
/****************************************************************/


/* TREATMENT EFFECTS */
program define rbicopula_tmeffects, eclass
	
	version 11
	syntax [anything] [if] [in] [pw fw iw] [, 		/*
				*/	TMEFFect(string) VCE(passthru) NOWEIGHTs NOEsample	/*
				*/	force Level(cilevel) post	/*
				*/	*]
	...
	if "`margopts0'" != ""{
		...
		dis as res	"See {help rbicopula tmeffects}." 
		exit 198
	}
	...
	*!	currently not applicable. future update will include
	if inlist("`e(copula)'","gaussian","gumbel","joe","amh"){
		dis as err "{bf:copula(`e(copula)')} currently not applicable for {bf:rbicopula tmeffects}"
		dis as txt "Next update of {bf:rbicopula} will provide application."
		exit 198
	}
	
	
	*!	ate, atet, and atec estimations
	tempname margfailed
	...
	*! adjust output table		
	if "`post'" == ""{
		...
		*!	Extract "b", "V", and "Cns" from matrices of e()
...
program define TmEff_Disp, eclass
	...
	*! table header
	...
	if "`atet'" != ""{
		local label `"Pr(`dep1'=1,`dep2'=1|`dep2'=1) - Pr(`dep1'=1,`dep2'=1|`dep2'=0)"'
		dis 
...
program define TmEff_Rt, rclass
	...
	local cmdline	= "rbicopula tmeffects `if' `in', tmeffect(`ate'`atet'`atec') `post'"
