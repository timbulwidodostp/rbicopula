...
{viewerjumpto "Syntax" "rbicopula margdec##syntax"}{...}
{viewerjumpto "Description" "rbicopula margdec##description"}{...}
{viewerjumpto "Links to PDF documentation" "rbicopula margdec##linkspdf"}{...}
{viewerjumpto "Options" "rbicopula margdec##options"}{...}
{viewerjumpto "Stores Results" "rbicopula margdec##results"}{...}
{viewerjumpto "Examples" "rbicopula margdec##examples"}{...}
{hline}
{hi:help rbicopula margdec}{right:{browse "https://github.com/cobanomics/rbicopula":github.com/cobanomics/rbicopula}}
{hline}
{right:also see:  {help rbicopula postestimation}}

{title:Title}

{p2colset 5 26 26 2}{...}
{p2col: {cmd:rbicopula margdec} {hline 2}}Marginal means, predictive margins, and marginal effects after {cmd:rbicopula}
{p_end}
{p2colreset}{...}
...
{p 8 15 2}
{cmd:rbicopula margdec} {ifin} [{it:{help rbicopula margdec##weight:weight}}] [{cmd:,} 
	{it:{help rbicopula margdec##response_options:response_options}}
	{it:{help rbicopula margdec##options_table:options}}] 
...
{synopt:{opt noe:sample}}do not restrict {cmd:rbicopula margdec} to the estimation sample{p_end}
...
{synopt :{it:{help rbicopula margdec##display_options:display_options}}}control
...
model by {cmd:rbicopula} at fixed values of some covariates and averaging or otherwise
integrating over the remaining covariates.

{pstd}
The {cmd:rbicopula margdec} command estimates margins of responses for
...
variable {it:depvar}_en, please use {helpb rbicopula tmeffects}. {cmd:rbicopula margdec}
...
{phang2}
	{opt effect(total)}: {cmd:rbicopula margdec} reports derivatives of the response with
...
{phang2}
	{opt effect(direct)}: {cmd:rbicopula margdec} reports derivatives of the response with
...
{phang2}
	{opt effect(indirect)}: {cmd:rbicopula margdec} reports derivatives of the response with
	respect to {it:varlist} from {opth dydx(varlist)}, {opt eyex(varlist)}, 
	{opt dyex(varlist)}, or {opt eydx(varlist)}, considering only the incorporation of 
...
{phang} 
	...
	after {cmd:rbicopula}. {opt predict(pred_opt)} specifies the option to be specified 
	with the {cmd:predict} command to produce the variable that will be used as the
	response. For example, after estimation by {cmd:rbicopula}, you could specify 
...
{pmore}
	...
	allowed with {cmd:rbicopula margdec}.

{phang}
	{opth dydx(varlist)}, {opt eyex(varlist)}, {opt dyex(varlist)}, and {opt eydx(varlist)} 
	request that {cmd:rbicopula margdec} report derivatives of the response with respect 
...
{pmore}
	...
	or {opt eydx()} options are not  allowed with {cmd:rbicopula margdec}.
...
{phang2}
    ...
    may have been specified with {cmd:rbicopula}.
	 
{dlgtab:Advanced}

{phang}
	{opt noweights} specifies that any weights specified on the previous estimation command 
	be ignored by {opt rbicopula margdec}.  By default, {opt rbicopula margdec} uses the 
	weights specified in {opt rbicopula} to average responses and to compute summary
	statistics.  If weights are specified on the {opt rbicopula margdec} command, 
...
{phang}
	{opt noesample} specifies that {cmd:rbicopula margdec} not restrict its computations to the
...
{phang} 
	{opt force} instructs {cmd:rbicopula margdec} to proceed in some situations where it would
...
{phang} 
	{opt post} 
	causes {cmd:rbicopula margdec} to behave like a Stata estimation (e-class) command.
	{cmd:rbicopula margdec} posts the vector of estimated margins along with the
...
{phang2}{cmd:. rbicopula graduate = income i.roommate i.hsgpagrp,}
		{cmd: endog(program = i.campus i.scholar income i.hsgpagrp) cop(frank)}{p_end}
...
{phang2}{cmd:. rbicopula margdec, dydx(income) predict(p11) effect(total)}{p_end}
{phang2}{cmd:. rbicopula margdec, dydx(income) predict(p11) effect(direct)}{p_end}
{phang2}{cmd:. rbicopula margdec, dydx(income) predict(p11) effect(indirect)}{p_end}
...
{phang2}{cmd:. rbicopula margdec, dydx(*) predict(p10) effect(direct)}{p_end}
{phang2}{cmd:. marginsplot}{p_end}

{pstd}Compute average marginal effects of {cmd:hsgpagrp} on the marginal probabilities{p_end}
{phang2}{cmd:. rbicopula margdec, dydx(hsgpagrp) predict(pmarg1)}{p_end}
{phang2}{cmd:. rbicopula margdec, dydx(hsgpagrp) predict(pmarg2)}{p_end}
...
{pstd}
{cmd:rbicopula margdec} stores the following in {cmd:r()}:
...
{pstd}
{cmd:rbicopula margdec} with the {cmd:post} option also stores the following in {cmd:e()}:
...
{psee}
    Online: help for
    {helpb rbicopula}, {helpb rbicopula postestimation}, {helpb margins}
