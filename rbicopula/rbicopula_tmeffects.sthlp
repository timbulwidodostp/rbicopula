...
{viewerjumpto "Syntax" "rbicopula tmeffects##syntax"}{...}
{viewerjumpto "Description" "rbicopula tmeffects##description"}{...}
{viewerjumpto "Options" "rbicopula tmeffects##options"}{...}
{viewerjumpto "Stores Results" "rbicopula tmeffects##results"}{...}
{viewerjumpto "Examples" "rbicopula tmeffects##examples"}{...}
{hline}
{hi:help rbicopula tmeffects}{right:{browse "https://github.com/cobanomics/rbicopula":github.com/cobanomics/rbicopula}}
{hline}
...
{p2colset 5 28 28 2}{...}
{p2col: {cmd:rbicopula tmeffects} {hline 2}}Estimation of treatment effects of treatment variable {it:depvar_en} after {cmd:rbicopula}
{p_end}
{p2colreset}{...}
...
{p 8 15 2}
{cmd:rbicopula tmeffects} {ifin} [{it:{help rbicopula tmeffects##weight:weight}}] [{cmd:,}
	{it:{help rbicopula tmeffects##options_table:options}}] 
...
{synopt:{opt noe:sample}}do not restrict {cmd:rbicopula tmeffects} to the estimation sample{p_end}
...
{synopt :{it:{help rbicopula tmeffects##display_options:display_options}}}control
...
{pstd}
{cmd: rbicopula tmeffects} estimates the average treatment effect, average treatment
...
{phang2}
	{opt tmeffect(ate)}: {cmd:rbicopula tmeffects} reports the average treatment effect,
...
{phang2}
	{opt tmeffect(atet)}: {cmd:rbicopula tmeffects} reports the average treatment effect
	on the treated, i.e. the finite difference between Pr({it:depvar}=1,{it:depvar}_en=1) 
	given {it:depvar}_en=1 and Pr({it:depvar}=1,{it:depvar}_en=1) given {it:depvar}_en=0, 
	computed and averaged only for the treated observations. Thus, {it:atet} is the difference 
	between the joint probability of outcome and treatment success conditioned on treatment success 
	and the joint probability of outcome and treatment success conditioned on treatment failure.

{phang2}
	{opt tmeffect(atec)}: {cmd:rbicopula tmeffects} reports the average treatment effect
...
{pmore}
    Multiple {opt tmeffect()} options are not allowed with {cmd:rbicopula tmeffects}.
...
{phang}
	...
	be ignored by {opt rbicopula tmeffects}.  By default, {opt rbicopula tmeffects} uses the 
	weights specified in {opt rbicopula} to average responses and to compute summary
	statistics.  If weights are specified on the {opt rbicopula tmeffects} command, 
...
{phang}
	{opt noesample} specifies that {cmd:rbicopula tmeffects} not restrict its computations to the
...
{phang} 
	{opt force} instructs {cmd:rbicopula tmeffects} to proceed in some situations where it would
...
{phang} 
	{opt post} 
	causes {cmd:rbicopula tmeffects} to behave like a Stata estimation (e-class) command.
	{cmd:rbicopula tmeffects} posts the vector of estimated margins along with the
...
{pstd}
{cmd:rbicopula tmeffects} stores the following in {cmd:r()}:
...
{pstd}
{cmd:rbicopula tmeffects} with the {cmd:post} option also stores the following in {cmd:e()}:
