...
/****************************************************************/
/*     rbicopula margdec: rbicopula postestimation command		*/
/****************************************************************/



/* MARGINS */
program define rbicopula_margdec, eclass
	...
	*!	parse original rbicopula
	local rbicop_cmdl = "`e(cmdline)'"
	local rbicop_cmdl: 	list retokenize rbicop_cmdl
	local pfxpos: 		list posof "rbicopula" in rbicop_cmdl
	
	local pfxsub ""
	forvalue x = 1(1)`pfxpos'{
		local wd: word `x' of `rbicop_cmdl'
		local pfxsub = "`pfxsub' `wd'"
	}
	local pfxsub: 	list retokenize pfxsub
	
	local 0 = subinstr("`rbicop_cmdl'","`pfxsub'","",1)
	local pfxcmd = subinstr("`pfxsub'","rbicopula","",1)
	...
	*! parse indepvars
	...
	tempvar rbicop_touse
	qui gen		`rbicop_touse' = 1		if e(sample)
	qui replace `rbicop_touse' = 0		if `rbicop_touse' == .
	...
	*!	parse rbiprobit margdec
	...
	syntax [anything] [if] [in] [pw fw iw], [/*
		*/	EFFect(string) PRedict(string) 		/*
		*/	dydx(passthru) eyex(passthru) eydx(passthru) dyex(passthru)	/*
		*/	VCE(passthru) NOWEIGHTs NOEsample force Level(cilevel) post /*
		*/	*]
	...
	if "`margopts0'" != ""{
		...
		dis as res	"See {help rbicopula margdec}." 
		exit 198
	}
	...
	*! parse dydx(), eyex(), dyex(), or eydx()
	if "`dydx'`eyex'`dyex'`eydx'" != ""{
		...
		tempname rbicop_beta
		matrix `rbicop_beta' = e(b)
		...
		ParseDyDx, `dydx' `eyex' `dyex' `eydx' ///
					dep2(`dep2') dep2n(`dep2n') matrix(`rbicop_beta') `dydxeffect'
	}
	...
	*! total marginal effect		
	if "`effect'" == "total"{
		...
		if `margfailed' == 0{
			...
			local marg_cmdl		= "rbicopula margdec `0'"
	...
	*! get indepvars and indepvars_en
	ParseOrigIndep, matrix(`rbicop_beta') dep2(`dep2')
	...
	*! direct marginal effect
	if "`effect'" == "direct"{		
		qui{
			`pfxcmd' rbicopula	`dep1' = `indep1' `ifcmd' `incmd' `wgtcmd', ///
		...
		*! indirect marginal effect	
		qui{
			`pfxcmd' rbicopula	`dep1' = `indep1new' `ifcmd' `incmd' `wgtcmd', ///
	...
	*! adjust e() and r()
	...
	if `margfailed' != 0{
		...
		if "`post'" == "post"{
			...
			MargErt, 	clonelist(`clonelist') origlist(`origlist') /*
						*/ jacvec(`jacvec') /*
						*/ cmdl("`marg_cmdl'") estcmdl(`rbicop_cmdl') bascmd(`marg_bascmd')
...
program define ParseDyDx, rclass
	...
	if "`test'" != ""{
		...
		dis as res	"Please use {help rbicopula tmeffects} for estimation of treatment effects."
		exit 198
	}				
